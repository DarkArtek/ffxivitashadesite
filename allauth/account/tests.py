...
class AccountTests(TestCase):
    ...
    def test_redirect_when_authenticated(self):
        ...
        self.assertRedirects(
            resp, "/account/profile/", fetch_redirect_response=False
        )
    ...
    def test_login_inactive_account(self):
        """
        Tests login behavior with inactive account.

        Inactive user account should be prevented from performing any actions,
    ...
    def test_ajax_login_success(self):
        ...
        self.assertEqual(resp.status_code, 200)
        data = json.loads(resp.content.decode('utf8'))
        self.assertEqual(data['location'], '/account/profile/')
